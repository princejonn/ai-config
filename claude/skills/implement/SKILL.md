---
name: implement
description: "Implements one defined code change in any language — feature, refactor, bug fix, scaffolding — to production quality with its tests and the project's full gate. Use for any code change, inline for a one-liner tiered trivial, otherwise in a developer agent; when the brief carries a plan, its stop conditions bind. Not for tests as the deliverable (test), finding a cause (diagnose-root-cause) or design (design-surface)."
---
