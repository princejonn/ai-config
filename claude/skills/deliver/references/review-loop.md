...
- **Re-review scope is the reviewer's contract**, `review` § Re-review and the adjudication ledger. The change-set-wide audit belongs to the first round and runs again only after a gate closes and its change-set lands — a new item against the landed tree.
...
- **A mechanism-wide fix** ("every site of X") carries an exhaustive site list produced by a `/verify` lane, with a per-site disposition the fix returns completed; the unswept remainder is the next round's finding.
