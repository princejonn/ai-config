---
name: deliver
description: "Runs an agreed item to committable code through agents: tiering, one living brief with its ledgers, review rounds to acceptance, a circuit breaker. Use once work is agreed; questions belong to design-surface or plan-phases."
---
...
- Once the work is agreed, run it to the end and report; questions belong to `design-surface` or `plan-phases`.
...
- A load-bearing claim goes through `/verify-claim` before it enters a brief or a verdict. Bulk reading goes through `/research`, an interpreting read goes through `/research-complex`, and the memo, not the material, enters the brief.
- A fresh reviewer each round on the living brief, the tier's per `references/tiers.md`: `reviewer` through `/review-change`, `reviewer-complex` dispatched by the chat. The developer works the fixes it returns.
