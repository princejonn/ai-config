...
- One attempt per item, never per round, and never a retry loop.
...
Write the packet to a file under `$TMPDIR`; it carries the material alone, and `bin/second-opinion-codex` prepends the mode's contract.
...
These never leave the machine; `bin/second-opinion-codex` refuses the packet and names the matching path:

- Secrets and credentials.
- Customer or personal data.
- Any path the repository marks confidential, one path or glob per line in `.confidential` at its root.
...
- `1`: unavailable — relay the `second-opinion-codex unavailable:` line and stop. `Connection failed` or `certificate was not trusted` is the sandbox, not a quota; report it rather than bypassing the sandbox.
