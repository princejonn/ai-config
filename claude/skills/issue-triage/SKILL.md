...
- split — § Related as the body edit: `gh issue comment <N> --body-file <path>` with the fix list, saying the original returns to the queue when its pieces close and that § Done then closes it against their commits; nothing closes it on its own. The pieces are created through `issue-write` carrying no dependency line, each filed as a sub-issue of the original, then `gh issue edit <N> --body-file <path>` gives the original's § Related one dependency line naming the pieces, in the form `issue-write` § Shape gives, with the blocked-by relation that line stands for, so `issue-next` skips it until they close. A piece `issue-write` § Refuse would refuse makes the row a question instead, ranked where split stands.
