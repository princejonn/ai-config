...
check $? "17 second apply reports AGENTS.md ok and leaves it byte-identical"
...
check $? "18 editing a rule regenerates AGENTS.md with the new content"
echo "plain rule" > "$FIX/claude/rules/plain.md"
...
check $? "19 foreign AGENTS.md without the marker is a CONFLICT and untouched"
...
check $? "20 symlinked AGENTS.md is a CONFLICT and left alone even when its content matches"
...
check $? "21 absent CODEX_HOME is skipped, nothing is created and check passes"
...
check $? "22 check with AGENTS.md pending exits 1 and writes nothing"

run_apply
apply_status=$status
run_apply --check
[ "$apply_status" = 0 ] && [ "$status" = 0 ] && cmp -s "$AGENTS_MD" "$CODEX_DIR/AGENTS.md"
check $? "23 check after apply exits 0"
...
check $? "24 foreign file in the agents skills dir is reported and fails check"
...
check $? "25 global targets never include a rule-set file such as code-style.md"
...
check $? "26 dangling owned links are pruned in both skills dirs"
...
[ "$apply_status" = 0 ] && [ "$status" = 1 ] && grep -q '^  settings: would change$' "$OUT" \
  && cmp -s "$WORK/settings.before" "$HOME_DIR/settings.json"
check $? "28 check reports a missing default as would change and writes nothing"
...
check $? "30 fresh apply links the second-opinion script into the local bin dir and warns once that the dir is not on PATH"
...
[ "$run_status" = 0 ] && [ "$status" = 0 ] \
  ...
  && ! grep -q '^  warn: ' "$OUT"
check $? "31 a local bin dir on PATH is reported ok without a warn line and passes check"
...
check $? "32 a dangling second-opinion link in the local bin dir is pruned and a link the repo does not own is left alone"
